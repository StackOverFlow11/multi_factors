"""VALLEY/RIDGE VWAP-RATIO factor (PR-J).
...
about the taxonomy is re-implemented here, so the four factors can never drift apart.
...
so a reader sees exactly what was assumed):
  ...
  3. POSITIVE-TRADE GUARD. A bar with non-finite or non-positive ``volume`` OR ``amount``
     ...
     baseline is PR-F's and must stay bit-identical. A zero-volume minute is therefore
     still classified (and still a valley) but contributes nothing to either VWAP.
