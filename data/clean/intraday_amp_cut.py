"""Intraday amplitude-cut factor (PR-G).
...
the 1min cache. Kept in the DATA-clean layer (like
:func:`data.clean.intraday_amplitude.compute_minute_ideal_amplitude`) so the ``factors``
layer only SELECTS the pre-aggregated column and never fetches or sees a forward return.
...
(invariant #1). This module is DATA-layer only: it does not fetch, does not touch
factors / alpha / portfolio / runtime, and never sees a token.
