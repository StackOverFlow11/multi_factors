"""PR-F runner: cache-only minute loader + the two-run evaluation core (no network)."""
...
from qt.eval_volume_peak_count import (
    _load_volume_peak_count_panel,
    evaluate_two_runs,
    extract_metrics,
)
...
# Three constant background days (baseline) then a test day with two clean peaks (the
# 200-volume minutes at 09:32 and 09:35, both with mild neighbours).
_DAYS = ("2021-07-01", "2021-07-02", "2021-07-03")
_TEST_DAY = "2021-07-04"
_BG_VOLS = [100.0] * 6
_PEAK_VOLS = [100.0, 200.0, 100.0, 100.0, 200.0, 100.0]

# Small gates so a 4-day cache produces a value (baseline needs >= 2 prior obs).
_LOAD_KW = dict(
...
def test_minute_loader_is_cache_only_and_discloses_empty(tmp_path):
    ...
    logger = logging.getLogger("test.vpc.loader")
    ...
    # the test day carries the two-peak count (=2) on both covered symbols
    d = pd.Timestamp(_TEST_DAY)
    assert load.factor.loc[(d, "AAA.SZ")] == pytest.approx(2.0)
    assert load.factor.loc[(d, "BBB.SZ")] == pytest.approx(2.0)
...
def test_minute_loader_blocks_when_nothing_cached(tmp_path):
    ...
    with pytest.raises(ValueError, match="no requested symbol produced"):
        _load_volume_peak_count_panel(
            cfg, ["ZZZ.SZ"], spec, logging.getLogger("test.vpc.block"), **_LOAD_KW
        )
...
def _synthetic_panels(n_days=90, n_symbols=15, seed=7):
    ...
    factor = pd.Series(
        rng.standard_normal(len(idx)), index=idx, name="volume_peak_count_20"
    )
    book = pd.DataFrame(
        {
...
def test_no_book_run_never_reaches_adopt(tmp_path):
    ...
    reports = evaluate_two_runs(
        factor, VolumePeakCountFactor().spec, _eval_cfg(), price, book,
        universe_symbols=(),
        fee_rate=0.001,
        report_dir=tmp_path,
    )
