"""PR-C runner: cache-only minute loader + the two-run evaluation core (no network)."""
...
from qt.eval_jump_amount_corr import (
    _load_jump_factor_panel,
    evaluate_two_runs,
    extract_metrics,
)
...
# --------------------------------------------------------------------------- #
def _stored_rows(sym, day, amps, amounts):
    """Build STORED_COLUMNS-shaped 1min rows (open=100, amplitude a) for one session."""
...
def test_minute_loader_is_cache_only_and_discloses_empty(tmp_path):
    ...
    # Two symbols with cached minute bars across two trading days; a third symbol
    # has NO cached minute at all (must be disclosed as empty, never fetched).
    for sym in ("AAA.SZ", "BBB.SZ"):
        for day in ("2021-07-01", "2021-07-02"):
            store.upsert(INTRADAY_ENDPOINT, sym, "1min",
                         _stored_rows(sym, day, amps, amts), KEY_COLS)

    cfg = _min_config(root, "2021-07-01", "2021-07-02")
    spec = JumpAmountCorrFactor().spec
    logger = logging.getLogger("test.jac.loader")
    load = _load_jump_factor_panel(
        cfg, ["AAA.SZ", "BBB.SZ", "CCC.SZ"], spec, logger,
        lookback_days=20, min_pairs=2,
    )
...
def test_minute_loader_blocks_when_nothing_cached(tmp_path):
    ...
    with pytest.raises(ValueError, match="no requested symbol produced"):
        _load_jump_factor_panel(
            cfg, ["ZZZ.SZ"], spec, logging.getLogger("test.jac.block"),
            lookback_days=20, min_pairs=2,
        )
...
def _synthetic_panels(n_days=90, n_symbols=15, seed=7):
    ...
    factor = pd.Series(rng.standard_normal(len(idx)), index=idx, name="jump_amount_corr_20")
    book = pd.DataFrame(
        {
...
def test_no_book_run_never_reaches_adopt(tmp_path):
    ...
    reports = evaluate_two_runs(
        factor, JumpAmountCorrFactor().spec, _eval_cfg(), price, book,
        universe_symbols=(),
        fee_rate=0.001,
        report_dir=tmp_path,
    )
