...
- ✅ **Phase 3-7 独立样本验证**（分支 `p3-independent-value-lowvol-validation`，**stacked 在 P3-6 分支上**——验收门要求复用 P3-6 机器；P3-6 merge 后可 rebase/顺序 PR。**EXPLORATORY**）：把 value/低波发现从"同窗口 POST-HOC 对照"推进到真正独立 holdout。不加因子/alpha、不调参；P3-6 group/cost 逻辑与配置不变（旧 27 测试原样通过）。
...
- ✅ 质量门：`pytest` **372 passed**（P0=97 / P1=78 / P2-1=22 / P2-2=22 / P2-3=14 / P2-4=8 / P3-1=10 / P3-2=18 / P3-3=16 / P3-4=15 / P3-5=22 / P3-6=27 / P3-7=22+1 throttle）；`ruff` clean；`validate-config`（全部 10 配置）+ `run-phase0`（demo）均 OK。
