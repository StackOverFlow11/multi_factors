...
- ✅ **数据层 D3 — report-only 数据质量层**（**PR #45 已 merge 到 `main`**，含 review 一修复;纯库 + 测试,**report-only 非 cleaning**）：新增独立 `data/quality/` 包,在接入处附近**只报告**可疑上游数据,**绝不**过滤行/修复值/改 qfq/改 cache coverage/动 feed·factor·alpha·portfolio·runtime。
  ...
  - **范围克制**:scope-smell 扫描 0 hits(质量层不碰 runtime/alpha/portfolio/cache 内部);**可选 `data-update` 集成推迟到 D3b**(本阶段仅库 + 测试,零 config/命令行为变更);phase0 `0.9600/0.8408` 不变。
- ✅ 质量门：`pytest` **625 passed**（P0=97 / P1=78 / P2-1=22 / P2-2=22 / P2-3=14 / P2-4=8 / P3-1=10 / P3-2=18 / P3-3=16 / P3-4=15 / P3-5=22 / P3-6=27 / P3-7=25+1 throttle / P3-8=8 / P4-1=28 / P4-2=17 / **intraday I1 schema=14 + feed=9 / I2 cache=10 / I3 aggregate=13 / I4 execution=10 / P4-3 cache=10 + updater=7 / P-I5a event-backtest=19 / P-I5b exec-feasibility=16 / P-I5c mmp-minute=18 / P-I5d mmp-quintile=19 / D1 token-dedup index-feed +2 / D2 cache-modularization +5 / D3 data-quality market=15 + intraday=12 + report=14**）；`ruff` clean；`validate-config`（全部 17 配置含 data_update + phase_i5a + phase_i5b + phase_i5c + phase_i5d）+ `run-phase0`（demo）均 OK（**ic 0.9600 / annual 0.8408 不变**）。
