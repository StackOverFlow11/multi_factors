...
- ✅ **分钟级 intraday pipeline I1–I4**（**PR #29 已 merge 到 `main`**，4 commit 一 PR；**全程与日频链路解耦——`factors`/`alpha`/`portfolio`、日频 `runtime/backtest`、日频 `TushareFeed`/`TushareCache`、全部 `config/` 零改动**，Phase 0/2/3 数字不变）。本地验收文档在 `tmp/context/intraday_pit_checkpoints/stage_i{1,2,3,4}_acceptance.md`（gitignored）。
  ...
  - **下一步 I5**：把 1~2 个 EXPLORATORY 分钟因子端到端接 I2→I3→I4（opt-in config，报告披露 cutoff/lag/execution_model/window），日频回归不破。
...
- ✅ 质量门：`pytest` **502 passed**（P0=97 / P1=78 / P2-1=22 / P2-2=22 / P2-3=14 / P2-4=8 / P3-1=10 / P3-2=18 / P3-3=16 / P3-4=15 / P3-5=22 / P3-6=27 / P3-7=25+1 throttle / P3-8=8 / P4-1=28 / P4-2=17 / **intraday I1 schema=14 + feed=9 / I2 cache=10 / I3 aggregate=13 / I4 execution=10 / P4-3 cache=10 + updater=7**）；`ruff` clean；`validate-config`（全部 13 配置含 data_update）+ `run-phase0`（demo）均 OK（**ic 0.9600 / annual 0.8408 不变**）。
...
- 路线图下一步：**分钟级轨道 I5**（把 EXPLORATORY 分钟因子端到端接 I2→I3→I4，opt-in config，报告披露 cutoff/lag/execution_model/window，日频回归不破）；**P4-3 follow-up**（fina 按 ann_date 建披露日历 ledger 以去掉启发式 tail / updater 加 stk_mins 历史回填 / data-update summary 落 cache-stats artifact）；研究侧：更长 holdout 滚动复检 / 中证1000 / 成本模型细化。
