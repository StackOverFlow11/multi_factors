...
- ✅ 质量门：`pytest` **538 passed**（P0=97 / P1=78 / P2-1=22 / P2-2=22 / P2-3=14 / P2-4=8 / P3-1=10 / P3-2=18 / P3-3=16 / P3-4=15 / P3-5=22 / P3-6=27 / P3-7=25+1 throttle / P3-8=8 / P4-1=28 / P4-2=17 / **intraday I1 schema=14 + feed=9 / I2 cache=10 / I3 aggregate=13 / I4 execution=10 / P4-3 cache=10 + updater=7 / P-I5a event-backtest=19 / P-I5b exec-feasibility=16**）；`ruff` clean；`validate-config`（全部 15 配置含 data_update + phase_i5a + phase_i5b）+ `run-phase0`（demo）均 OK（**ic 0.9600 / annual 0.8408 不变**）。
...
- 路线图下一步：**分钟级轨道 I5c**（执行真实性已就位：I5a 事件框架 + I5b raw `stk_limit` 执行期涨跌停 → 把 1~2 个 EXPLORATORY 分钟因子作为真实 opt-in alpha 端到端接 decision feature→alpha→portfolio→event engine，报告披露 cutoff/lag/execution_model/window，日频回归不破；首个分钟因子研究不会被后续可行性修复推翻）；**I5b follow-up**（执行期 feasibility 可再扩 partial-fill / liquidity / volume cap）；**P4-3 follow-up**（fina 按 ann_date 建披露日历 ledger 以去掉启发式 tail / updater 加 stk_mins 历史回填 / data-update summary 落 cache-stats artifact）；研究侧：更长 holdout 滚动复检 / 中证1000 / 成本模型细化。
