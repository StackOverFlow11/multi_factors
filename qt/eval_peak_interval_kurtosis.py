"""run-eval-peak-interval-kurtosis: the sixth real factor evaluation (PR-H).
...
"量峰间隔峰度" factor — as a first-class
:class:`~factors.compute.intraday_derived.PeakIntervalKurtosisFactor` and runs it through
the FROZEN :class:`~analytics.eval.StandardFactorEvaluator` on REAL cached A-share data
(CSI500, PIT membership) — the same contract-driven loop PR-C / PR-D / PR-E / PR-F /
PR-G used.

The factor is a DAILY signal derived DIRECTLY from the 1min cache (see
``data.clean.intraday_peak_interval.compute_peak_interval_kurtosis``): PIT-truncate each
day at 14:50, identify the volume peaks with the taxonomy REUSED from PR-F
(``data.clean.intraday_volume_prv``: same-slot strictly-prior μ+σ eruptive test, mild
1-minute same-session neighbours), measure the gaps between consecutive same-day peaks in
TRADING MINUTES, pool the trailing 20 VALID trading days and take the pool's Fisher
excess (bias-corrected) kurtosis. It is executed CLOSE-TO-CLOSE (daily default), so
``is_intraday=False`` (the reasoning is documented on the factor's spec). The eval CELL is
identical to PR-C..PR-G, so this run is directly comparable to its PR-F sibling — same
peaks, different statistic.
...
value_bp / volatility_20) so the Incremental axis measures whether the peak-interval
kurtosis adds alpha BEYOND value / low-vol.
"""
...
_LOGGER_NAME = "qt.eval_peak_interval_kurtosis"
_REPORT_STEM = "eval_peak_interval_kurtosis"
...
class _PeakIntervalMinuteLoad:
    """Diagnostics from the cache-only per-symbol minute read + aggregation."""

    factor: pd.Series  # MultiIndex(date, symbol) raw peak-interval kurtosis
    requested: int
...
def _load_peak_interval_kurtosis_panel(
    cfg: RootConfig,
    symbols: list[str],
    spec: FactorSpec,
    logger,
    *,
    ...
    min_intervals: int,
) -> _PeakIntervalMinuteLoad:
    """Compute the raw peak-interval-kurtosis panel per symbol from the minute cache.
    ...
    minute simply yields no rows and is disclosed as empty). The peak taxonomy (reused
    from PR-F) and the interval/kurtosis reduction run entirely inside
    ``compute_peak_interval_kurtosis`` on 1min bars.
    """
    ...
    for i, sym in enumerate(symbols):
        ...
        s = compute_peak_interval_kurtosis(
            bars,
    ...
    if not series:
        raise ValueError(
            "run-eval-peak-interval-kurtosis blocked: no requested symbol produced a "
            f"cached peak-interval-kurtosis value over [{cfg.data.start}, "
    ...
    return _PeakIntervalMinuteLoad(
        factor=factor,
...
def _build_eval_config(cfg: RootConfig) -> EvalConfig:
    ...
    if cfg.oos is None:
        raise ValueError(
            "run-eval-peak-interval-kurtosis requires an 'oos' section (split_date) so "
...
# --------------------------------------------------------------------------- #
@dataclass(frozen=True)
class PeakIntervalKurtosisEvalResult:
    """Immutable summary of one run-eval-peak-interval-kurtosis run."""
...
def _check_preconditions(cfg: RootConfig) -> None:
    ...
    if cfg.data.source != "tushare":
        raise ValueError(
            "run-eval-peak-interval-kurtosis needs data.source='tushare' (real cached "
            f"A-share data); got {cfg.data.source!r}."
        )
    if not cfg.data.cache.enabled:
        raise ValueError(
            "run-eval-peak-interval-kurtosis needs data.cache.enabled=true (it reads "
            "the persistent tushare cache and never warms live)."
        )
    if cfg.universe.type != "index":
        raise ValueError(
            "run-eval-peak-interval-kurtosis needs universe.type='index' (PIT "
            f"membership, e.g. 000905.SH for CSI500); got {cfg.universe.type!r}."
        )
    if not cfg.processing.neutralize.enabled:
        raise ValueError(
            "run-eval-peak-interval-kurtosis expects processing.neutralize.enabled=true "
            "(industry + size neutralization, matching the report's neutral column and "
            "the EvalConfig declaration)."
        )


def run_eval_peak_interval_kurtosis(config_path: str) -> PeakIntervalKurtosisEvalResult:
    ...
    factor = PeakIntervalKurtosisFactor(lookback_days=PEAK_INTERVAL_LOOKBACK_DAYS)
    spec = factor.spec
    ...
    # Peak-interval-kurtosis factor: cache-only per-symbol aggregation -> raw -> process.
    load = _load_peak_interval_kurtosis_panel(
        ...
        min_intervals=PEAK_INTERVAL_MIN_INTERVALS,
    )
    ...
    return PeakIntervalKurtosisEvalResult(
        config=cfg,
        spec=spec,
...
__all__ = [
    "PeakIntervalKurtosisEvalResult",
    "evaluate_two_runs",
    "extract_metrics",
    "run_eval_peak_interval_kurtosis",
]
