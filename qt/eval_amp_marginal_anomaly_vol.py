"""run-eval-amp-marginal-anomaly-vol: the third real factor evaluation (PR-E).

Reproduces the Changjiang high-frequency-factor series #19 (长江证券《高频因子（十九）》,
reportId 5462994) "amplitude marginal-anomaly relative-volatility" factor as a
first-class :class:`~factors.compute.intraday_derived.AmpMarginalAnomalyVolFactor` and
runs it through the FROZEN :class:`~analytics.eval.StandardFactorEvaluator` on REAL
cached A-share data (CSI500, PIT membership) — the same contract-driven loop PR-C /
PR-D used.

The factor is a DAILY signal derived from 5min bars (themselves DERIVED from the 1min
cache; see ``data.clean.intraday_amp_anomaly.compute_amp_marginal_anomaly_vol``): pool
the trailing 20 trading days of PIT-truncated (14:50) 5min bars, form within-day
``(|Δamp|, r)`` pairs, select the bars whose ``|Δamp| > μ + σ`` of the pooled
``|Δamp|``, and take the ddof=1 std of the RETURNS on those selected bars. It is
executed CLOSE-TO-CLOSE (daily default), so ``is_intraday=False`` (the reasoning is
...
value_bp / volatility_20) so the Incremental axis measures whether the anomaly-vol
...
_LOGGER_NAME = "qt.eval_amp_marginal_anomaly_vol"
_REPORT_STEM = "eval_amp_marginal_anomaly_vol"
...
class _AnomalyVolMinuteLoad:
    """Diagnostics from the cache-only per-symbol minute read + aggregation."""

    factor: pd.Series  # MultiIndex(date, symbol) raw amp-marginal-anomaly-vol
    requested: int
...
def _load_amp_anomaly_vol_panel(
    cfg: RootConfig,
    symbols: list[str],
    spec: FactorSpec,
    logger,
    *,
    ...
    min_selected: int,
    sigma_k: float,
) -> _AnomalyVolMinuteLoad:
    """Compute the raw amp-marginal-anomaly-vol panel per symbol from the minute cache.
    ...
    minute simply yields no rows and is disclosed as empty). The 1min -> 5min derivation
    happens inside ``compute_amp_marginal_anomaly_vol``.
    """
    ...
    for i, sym in enumerate(symbols):
        ...
        s = compute_amp_marginal_anomaly_vol(
            bars,
    ...
    if not series:
        raise ValueError(
            "run-eval-amp-marginal-anomaly-vol blocked: no requested symbol produced a "
            f"cached amp-marginal-anomaly-vol value over [{cfg.data.start}, "
...
def _build_eval_config(cfg: RootConfig) -> EvalConfig:
    ...
    if cfg.oos is None:
        raise ValueError(
            "run-eval-amp-marginal-anomaly-vol requires an 'oos' section (split_date) "
...
# --------------------------------------------------------------------------- #
@dataclass(frozen=True)
class AmpAnomalyVolEvalResult:
    """Immutable summary of one run-eval-amp-marginal-anomaly-vol run."""
...
def _check_preconditions(cfg: RootConfig) -> None:
    ...
    if cfg.data.source != "tushare":
        raise ValueError(
            "run-eval-amp-marginal-anomaly-vol needs data.source='tushare' (real cached "
            f"A-share data); got {cfg.data.source!r}."
        )
    if not cfg.data.cache.enabled:
        raise ValueError(
            "run-eval-amp-marginal-anomaly-vol needs data.cache.enabled=true (it reads "
            "the persistent tushare cache and never warms live)."
        )
    if cfg.universe.type != "index":
        raise ValueError(
            "run-eval-amp-marginal-anomaly-vol needs universe.type='index' (PIT "
            f"membership, e.g. 000905.SH for CSI500); got {cfg.universe.type!r}."
        )
    if not cfg.processing.neutralize.enabled:
        raise ValueError(
            "run-eval-amp-marginal-anomaly-vol expects processing.neutralize.enabled="
            "true (industry + size neutralization, matching the report's neutral column "
            "and the EvalConfig declaration)."
        )


def run_eval_amp_marginal_anomaly_vol(config_path: str) -> AmpAnomalyVolEvalResult:
    """Run the two real amp-marginal-anomaly-vol evaluations (cache-only) + reports."""
    ...
    factor = AmpMarginalAnomalyVolFactor(lookback_days=AMP_ANOMALY_LOOKBACK_DAYS)
    spec = factor.spec
    ...
    # Amp-marginal-anomaly-vol factor: cache-only per-symbol aggregation -> raw -> process.
    load = _load_amp_anomaly_vol_panel(
        cfg, symbols, spec, logger,
        lookback_days=AMP_ANOMALY_LOOKBACK_DAYS,
        min_pool=AMP_ANOMALY_MIN_POOL,
        min_selected=AMP_ANOMALY_MIN_SELECTED,
        sigma_k=AMP_ANOMALY_SIGMA_K,
    )
    ...
    return AmpAnomalyVolEvalResult(
        config=cfg,
        spec=spec,
...
__all__ = [
    "AmpAnomalyVolEvalResult",
    "evaluate_two_runs",
    "extract_metrics",
    "run_eval_amp_marginal_anomaly_vol",
]
