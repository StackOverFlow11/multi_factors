"""run-eval-volume-peak-count: the fourth real factor evaluation (PR-F).

Reproduces the Kaiyuan market-microstructure series #27 (开源证券《高频成交量的峰、岭、
谷信息——市场微观结构研究系列（27）》, reportId 4957417) flagship "volume-peak-minute-count"
factor as a first-class
:class:`~factors.compute.intraday_derived.VolumePeakCountFactor` and runs it through the
FROZEN :class:`~analytics.eval.StandardFactorEvaluator` on REAL cached A-share data
(CSI500, PIT membership) — the same contract-driven loop PR-C / PR-D / PR-E used.

The factor is a DAILY signal derived DIRECTLY from the 1min cache (no coarser resample;
see ``data.clean.intraday_volume_prv.compute_volume_peak_count``): PIT-truncate each day
at 14:50, classify every visible minute against its SAME-SLOT strictly-prior 20-day
baseline (eruptive if ``vol > μ + σ`` else mild), mark the eruptive minutes whose both
1-minute same-session neighbours are mild as PEAKS, and count the peak minutes over the
trailing 20 VALID trading days. It is executed CLOSE-TO-CLOSE (daily default), so
...
value_bp / volatility_20) so the Incremental axis measures whether the volume-peak-count
...
_LOGGER_NAME = "qt.eval_volume_peak_count"
_REPORT_STEM = "eval_volume_peak_count"
...
class _VolumePeakMinuteLoad:
    """Diagnostics from the cache-only per-symbol minute read + aggregation."""

    factor: pd.Series  # MultiIndex(date, symbol) raw volume-peak-count
    requested: int
...
def _load_volume_peak_count_panel(
    cfg: RootConfig,
    symbols: list[str],
    spec: FactorSpec,
    logger,
    *,
...
) -> _VolumePeakMinuteLoad:
    """Compute the raw volume-peak-count panel per symbol from the minute cache.
    ...
    minute simply yields no rows and is disclosed as empty). The same-slot baseline /
    peak taxonomy runs entirely inside ``compute_volume_peak_count`` on 1min bars.
    """
    ...
    if not series:
        raise ValueError(
            "run-eval-volume-peak-count blocked: no requested symbol produced a cached "
            f"volume-peak-count value over [{cfg.data.start}, {cfg.data.end}]. The "
...
def _build_eval_config(cfg: RootConfig) -> EvalConfig:
    ...
    if cfg.oos is None:
        raise ValueError(
            "run-eval-volume-peak-count requires an 'oos' section (split_date) so the "
...
# --------------------------------------------------------------------------- #
@dataclass(frozen=True)
class VolumePeakCountEvalResult:
    """Immutable summary of one run-eval-volume-peak-count run."""
...
def _check_preconditions(cfg: RootConfig) -> None:
    ...
    if cfg.data.source != "tushare":
        raise ValueError(
            "run-eval-volume-peak-count needs data.source='tushare' (real cached "
            f"A-share data); got {cfg.data.source!r}."
        )
    if not cfg.data.cache.enabled:
        raise ValueError(
            "run-eval-volume-peak-count needs data.cache.enabled=true (it reads the "
            "persistent tushare cache and never warms live)."
        )
    if cfg.universe.type != "index":
        raise ValueError(
            "run-eval-volume-peak-count needs universe.type='index' (PIT membership, "
            f"e.g. 000905.SH for CSI500); got {cfg.universe.type!r}."
        )
    if not cfg.processing.neutralize.enabled:
        raise ValueError(
            "run-eval-volume-peak-count expects processing.neutralize.enabled=true "
            "(industry + size neutralization, matching the report's neutral column and "
            "the EvalConfig declaration)."
        )


def run_eval_volume_peak_count(config_path: str) -> VolumePeakCountEvalResult:
    """Run the two real volume-peak-count evaluations (cache-only) + reports."""
    ...
    factor = VolumePeakCountFactor(lookback_days=VOLUME_PRV_LOOKBACK_DAYS)
    spec = factor.spec
    ...
    # Volume-peak-count factor: cache-only per-symbol aggregation -> raw -> process.
    load = _load_volume_peak_count_panel(
    ...
    return VolumePeakCountEvalResult(
        config=cfg,
        spec=spec,
...
__all__ = [
    "VolumePeakCountEvalResult",
    "evaluate_two_runs",
    "extract_metrics",
    "run_eval_volume_peak_count",
]
