...
def render_bias_audit() -> str:
    ...
    return (
        ...
        "- **涨跌停(UNI-007)**:close 触及当日 `up_limit`/`down_limit` 标记 "
        "`at_up_limit`/`at_down_limit`(实证:`000005.SZ` 2024-02-01 触跌停)。当前"
        "选股层对两个方向都剔除;**方向感知**(买入只看涨停、持有跌停不强卖)属执行层,"
        "后续细化。\n"
        ...
        "缺行业 / 市值或截面名称 < 3 时返回 NaN,**不静默乱算**;`processing.neutralize` "
        "开启但协变量缺失(如 demo 路径)直接报可读错误。\n"
