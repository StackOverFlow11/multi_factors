"""run-eval-jump-amount-corr: the FIRST real factor evaluation (PR-C).

Reproduces the Kaiyuan report §6 "price-jump turnover correlation" factor as a
first-class :class:`~factors.compute.intraday_derived.JumpAmountCorrFactor` and
runs it through the FROZEN :class:`~analytics.eval.StandardFactorEvaluator` on REAL
cached A-share data (CSI500, PIT membership), closing the contract-driven loop.

The factor is a DAILY signal derived from 1min bars (see
``data.clean.intraday_aggregate.compute_jump_amount_corr``): the trailing-20-
trading-day lagged correlation between the amount at price-jump minutes and the
amount at the strictly-next minute. It is executed CLOSE-TO-CLOSE (daily default),
so ``is_intraday=False`` (the reasoning is documented on the factor's spec).
...
universe / covariate endpoints go through the shared read-through cache, which on
a fully-warmed cache does zero gap fetches (disclosed via the run-log cache-stats
...
The evaluator is run TWICE: once with NO known-factor book (the Incremental axis
is NOT_ASSESSED) and once with the project's independently-confirmed book
(value_ep / value_bp / volatility_20) so the Incremental axis measures whether
jump-amount-corr adds alpha BEYOND value / low-vol.
"""
...
_LOGGER_NAME = "qt.eval_jump_amount_corr"
_REPORT_STEM = "eval_jump_amount_corr"
...
class _JumpMinuteLoad:
    """Diagnostics from the cache-only per-symbol minute read + aggregation."""

    factor: pd.Series  # MultiIndex(date, symbol) raw jump-amount-corr
    requested: int
...
def _load_jump_factor_panel(
    ...
    *,
    ...
    """Compute the raw jump-amount-corr panel per symbol from the minute cache.
    ...
    has no fetch closure, so ``stk_mins`` live calls are provably zero (a symbol
    with no cached minute simply yields no rows and is disclosed as empty).
    """
    ...
    for i, sym in enumerate(symbols):
        ...
        s = compute_jump_amount_corr(
            bars, lookback_days=lookback_days, min_pairs=min_pairs, name=spec.factor_id
        )
        if s.notna().any():
            series.append(s)
            covered.append(sym)
    ...
    if not series:
        raise ValueError(
            "run-eval-jump-amount-corr blocked: no requested symbol produced a "
            f"cached minute jump-amount-corr value over [{cfg.data.start}, "
...
def evaluate_two_runs(
    ...
    *,
    ...
    panel (for forward returns), and the PROCESSED known-factor book, it does the
    two ``evaluate`` calls and writes ``{stem}_no_book`` / ``{stem}_with_book`` as
...
def _build_eval_config(cfg: RootConfig) -> EvalConfig:
    ...
    config may toggle it. z-score + industry/size neutralization ARE applied, so
    they are declared. ``oos_split`` (from the config's ``oos`` block) makes the
    OOS section run so the Predictive axis can be assessed. ``is_exploratory=True``:
    this is a reproduction on a shorter window / narrower neutralization than the
    report, not a return claim (it caps the deployment label at Watch).
    """
    if cfg.oos is None:
        raise ValueError(
            "run-eval-jump-amount-corr requires an 'oos' section (split_date) so the "
    ...
    return EvalConfig(
        ...
        tuned=False,
        # We evaluated ONE pre-registered factor whose sign came from the report
        # (not a screen of our own); the report's own 17-factor screen is a caveat
        # noted in the run's prose, not our multiple-testing background.
...
# --------------------------------------------------------------------------- #
@dataclass(frozen=True)
class JumpEvalResult:
    """Immutable summary of one run-eval-jump-amount-corr run."""
...
def _check_preconditions(cfg: RootConfig) -> None:
    ...
    if cfg.data.source != "tushare":
        raise ValueError(
            "run-eval-jump-amount-corr needs data.source='tushare' (real cached "
            f"A-share data); got {cfg.data.source!r}."
        )
    if not cfg.data.cache.enabled:
        raise ValueError(
            "run-eval-jump-amount-corr needs data.cache.enabled=true (it reads the "
            "persistent tushare cache and never warms live)."
        )
    if cfg.universe.type != "index":
        raise ValueError(
            "run-eval-jump-amount-corr needs universe.type='index' (PIT membership, "
            f"e.g. 000905.SH for CSI500); got {cfg.universe.type!r}."
        )
    if not cfg.processing.neutralize.enabled:
        raise ValueError(
            "run-eval-jump-amount-corr expects processing.neutralize.enabled=true "
            "(industry + size neutralization, matching the report's neutral column "
            "and the EvalConfig declaration)."
        )


def run_eval_jump_amount_corr(config_path: str) -> JumpEvalResult:
    """Run the two real jump-amount-corr evaluations (cache-only) and write reports."""
    ...
    factor = JumpAmountCorrFactor(lookback_days=JUMP_LOOKBACK_DAYS)
    spec = factor.spec
    eval_cfg = _build_eval_config(cfg)
    logger.info("eval config: %s rebalance=daily oos_split=%s", eval_cfg.universe, eval_cfg.oos_split)

    cache = _build_cache(cfg)
    universe, symbols = _build_universe(cfg, logger, cache)
    panel = _load_panel(cfg, symbols, logger, cache)

    # Book (value_ep / value_bp / volatility_20): enrich, compute, process. The
    # value factors need daily_basic pe/pb; volatility_20 needs close.
    ...
    # Jump factor: cache-only per-symbol minute aggregation -> raw -> process.
    load = _load_jump_factor_panel(
        cfg, symbols, spec, logger,
        lookback_days=JUMP_LOOKBACK_DAYS, min_pairs=JUMP_MIN_PAIRS,
    )
    # A minute date with no daily bar cannot have a forward return; keep the factor
    # on the daily trading grid so the analytics boundary has a price for every date.
    jump_raw = load.factor[
        load.factor.index.get_level_values(DATE_LEVEL).isin(panel_dates)
    ]
    jump_processed = _process_factors(cfg, jump_raw.to_frame(spec.factor_id), panel)
    factor_series = jump_processed[spec.factor_id]
