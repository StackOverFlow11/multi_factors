...
⚠️ **限频未逐个压测**：返回数据的接口只证明「这一次能调」，**未测每分钟/每天上限**。其中
`stk_mins`、`hm_detail` 因本会话先前 MCP 探测已各消耗掉额度，SDK 复测即暴露 `1 次/小时`；
...
| 接口 | 用途 | SDK 结果 | 判定 | 说明 |
|---|---|---|---|---|
| `stk_mins` | **分钟线** 1/5/15/30/60min | `频率超限(1次/小时)` | ⚠️ **callable but NOT batch-viable** | 有基础权限但限 **1 次/小时**，对全市场分钟 ETL **等于不可用**；要做分钟级须**单独购买更高分钟权限档**放开频次 |
| `hm_detail` | 游资交易每日明细 | `频率超限(1次/小时)` | ⚠️ **callable but NOT batch-viable** | 同上，限 **1 次/小时** |
...
- **有权限但限 1 次/小时、不可批量**：`stk_mins`、`hm_detail`——分钟级研究仍需单独购买分钟权限。
- **未压测限频（authorized 但批量额度未知）**：全部 EXPLORATORY 候选源——接入前各做一次小批量限频体检
