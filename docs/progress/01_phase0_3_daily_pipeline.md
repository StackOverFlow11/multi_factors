# Phase 0 – Phase 3：日频截面多因子链路（PR #1 – #21）

> 本文件是 `CLAUDE.md` 进度日志的**归档**，内容逐字搬移，未做任何改写。
> 索引见 [`docs/progress/README.md`](README.md)；当前状态与操作约定仍在仓库根 `CLAUDE.md`。

从 7 层骨架 / Phase 0 MVP，到 Phase 1 偏差边界、Phase 2 执行真实性与 PIT 行业、Phase 3 多因子 baseline / walk-forward IC 加权 / OOS / robustness matrix / 候选因子组 / 子集复检 / 独立样本验证 / CSI500 泛化。
