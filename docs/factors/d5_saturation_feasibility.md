...
| 因子 | 输出行 | 有值 | 出现的票 | 每次请求票数 | 名义最大帧 | **峰值 RSS** | wall | live_calls |
|---|---|---|---|---|---|---|---|---|
| `intraday_amp_cut_10` | 1,158,842 | 1,158,842 | **995/995** | **1** | 0.0656 GB | **0.964 GB** | 1990.5 s | **0** |

**995/995**：没有任何一只被请求的票在流式化后消失（I5a 红线）。`live_calls=0` 全程
cache-only。其余因子的 sweep 可用同一命令续跑（JSONL 已记录的因子会被跳过）。
