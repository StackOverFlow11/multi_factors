...
收紧后仍 18/18，但守卫从"几乎恒真"变成"能失败"。（mutation 实证：把门槛放回中位 → 仍全过，
说明 p25 是**严格更紧**且当前数据两边都清关；往白名单里塞一只三因子都稠密的票 `000009.SZ`
（48/41、49/41、47/40）→ **p25 门下仍然转红**。）
...
「成员判据是机制」与「这组参数是怎么被推错、又怎么重新推的」两段。）
