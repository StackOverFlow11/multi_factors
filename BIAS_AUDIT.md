...
- **行业 PIT(UNI-010,P2-3,已实现)**:行业协变量从 `stock_basic.industry` 的**当前**标签,升级为按 trade_date **as-of** 的历史申万一级(SW-L1)。`data.feed.tushare_covariates.pit_sw_l1_intervals` 读 `index_member_all` 拿每股 SW-L1 的 `in_date`/`out_date` 区间;`data.clean.pit_industry.asof_industry` 按`[in_date, out_date)` 覆盖该日的区间取行业(改分类日**新行业**生效,PIT-safe,绝不用未来行业)。起始日前已有的成分能 carry forward 到窗口开始。
...
- **残留**:SW-L1 历史采用当前 SW2021 分类体系的 in/out 记录;对稳定大盘股,as-of 行业与当前标签通常一致(行为中性),但**边界已 PIT-safe**(改分类的票各按其时代归属)。
